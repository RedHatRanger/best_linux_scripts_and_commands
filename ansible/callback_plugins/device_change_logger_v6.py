...
class CallbackModule(CallbackBase):
    ...
    In check mode (-C), logs to a separate 'check/changes.csv' file by default and prefixes summarized changes with 'Check mode: '.
    ...
    def _log_change(self, result):
        ...
        In check mode (-C), logs to ~/ansible/changes/check/changes.csv (if using default path) and prefixes summarized changes with 'Check mode: '.
        Args:
        ...
        # Determine log file path: use 'check/changes.csv' in check mode if default path
        log_file = self.log_file
        if result._result.get('check_mode', False) and log_file == os.path.join(self.log_dir, 'change_logs.csv'):
            log_file = os.path.join(self.log_dir, 'check', 'changes.csv')  # Use check subdirectory
            # Initialize check mode CSV if it doesn't exist
            if not os.path.exists(log_file):
                os.makedirs(os.path.dirname(log_file), exist_ok=True)  # Creates check subdirectory
        ...
        # Format summarized changes (human-readable) and detailed changes (filtered JSON)
        ...
        filtered_result = {k: v for k, v in result._result.items() if k not in ['ansible_facts', 'invocation']}  # Exclude large fields
        detailed = json.dumps(filtered_result, sort_keys=True)  # Filtered task result as JSON
    ...
    def _format_change_details(self, result_dict):
        ...
        Processes 'before'/'after' (e.g., file attributes), 'diff' (e.g., line changes), 'commands' (e.g., cisco.ios.ios_config, cisco.ios.ios_snmp_server),
        'snmp', 'community', or 'snmp_server' (e.g., cisco.ios.ios_snmp_server settings).
        Falls back to JSON with detailed debugging if no known structure is found.
        Args:
        ...
        def recursive_diff(old, new, path=""):
            ...
            if isinstance(old, dict) and isinstance(new, dict):
                ...
                # For lists, show if lengths differ or items differ (simple check)
                if len(old) != len(new):
        ...
        try:
            # Handle 'commands' key first (common in cisco.ios.ios_config, cisco.ios.ios_snmp_server)
            commands = result_dict.get('commands')
            if commands and isinstance(commands, (list, str)):
                if isinstance(commands, str):
                    commands = [commands]  # Convert single string to list for consistency
                if commands:  # Ensure commands is not empty
                    return "Applied SNMP commands: " + "; ".join(str(cmd) for cmd in commands)
                return "No SNMP commands applied"

            # Handle 'before' and 'after' keys (common in ansible.builtin.file, template, resource modules)
            before = result_dict.get('before')
            after = result_dict.get('after')
            if before is not None and after is not None:
                # Use recursive diff for dictionary-based before/after (handles nested changes)
                if isinstance(before, dict) and isinstance(after, dict):
                    changes = recursive_diff(before, after)
                    if changes:
                        return "; ".join(changes)
                    return "No attribute changes detected"
                # List-based: compare lengths or items
            ...
            # Handle 'snmp' key (specific to cisco.ios.ios_snmp_server)
            ...
            # Handle 'community' key directly (possible in cisco.ios.ios_snmp_server)
            ...
            # Handle 'snmp_server' key (possible variant in cisco.ios.ios_snmp_server)
            ...
            # General fallback for any change: summarize non-standard keys in result_dict
            changes = []
            for k, v in result_dict.items():
                if k not in ['changed', 'check_mode', 'msg', 'invocation', 'before', 'after', 'diff', 'commands', 'snmp', 'community', 'snmp_server']:
                    changes.append(f"{k}: {str(v)[:100]}")  # Truncate long values
            if changes:
                return "General changes: " + "; ".join(changes)

            # Fallback with detailed debugging: log all keys and partial result
            debug_info = f"Unrecognized result structure. Keys found: {list(result_dict.keys())}; Partial result: {json.dumps(result_dict, sort_keys=True)[:150]}"
        ...
        for r in rows[1:]:
            ...
            for col in new_header:
                if col == 'Invoked By' and col not in header:
                    ...
                    # Use Change Details if available, else empty
                    ...
                    new_r.append('')  # Empty for new columns
